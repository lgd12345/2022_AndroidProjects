...
// 이미지 크기 계산
        val dr: Drawable? = ResourcesCompat.getDrawable(context.resources, R.drawable.star, null)
        val drWidth = dr?.intrinsicWidth
        val drHeight = dr?.intrinsicHeight
// 이미지를 출력할 위치 계산
        val left = width / 3 - drWidth?.div(3) as Int
        val top = height / 3 - drHeight?.div(3) as Int
// 이미지 출력
        c.drawBitmap(
...
// 뷰 크기 계산
        ...
        val height = parent.height
// 이미지 크기 계산
        val dr: Drawable? = ResourcesCompat.getDrawable(context.resources, R.drawable.c, null)
        val drWidth = dr?.intrinsicWidth
        val drHeight = dr?.intrinsicHeight
// 이미지를 출력할 위치 계산
        val left = width / 3 - drWidth?.div(3) as Int
...
// 이미지 출력
        c.drawBitmap(
            BitmapFactory.decodeResource(context.resources, R.drawable.c),
            left.toFloat(),
            top.toFloat(),
            null
        )
